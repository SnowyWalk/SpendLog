...
const TARGET_MONTH = 6; // 1~12

// "1": 전체조회, "0": 카드별 조회
const INQUIRY_TYPE = '1';

// "0": 가맹점정보 미포함, "1": 가맹점정보 포함
const MEMBER_STORE_INFO_TYPE = '0';

// CODEF 삼성카드 승인내역 API 필수값. "0"은 최신순 조회.
...
async function main() {
    ...
    console.log(`삼성카드 승인내역 조회: ${startDate} ~ ${endDate}`);
...
main().catch((error) => {
    console.error('삼성카드 승인내역 조회 중 오류 발생');
    console.error(error);
});
